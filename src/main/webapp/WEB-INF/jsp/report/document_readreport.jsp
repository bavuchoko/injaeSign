...
<%@ include file="../header.jsp" %>
	...
	<script>
		...
		<div id="righter">
		<form method="post" name="documentFRM">
		<input type="hidden" name="d_num" value="${details.d_num}">
			<div class="document">
			
				<div class="col-md-8">
					<h3>${details.d_code}</h3>
					<div class="confirm_btn" style="position : relative">
						<c:if test="${checker eq 'approved'}">
						<input type="button" value="기승인" style="background-color: #2b669a; color:white">

						</c:if>
						<c:if test="${checker eq 'rejected'}">
							<input type="button" value="기반려" style="background-color: brown; color:white">
						</c:if>
						<c:if test="${checker eq 'yet'}">
							<c:if test="${yet eq 'yes'}">
							<input class="approveBtn" type="button" value="승인">
							<input class="rejectBtn" type="button" value="반려">
						...
						<div class="qwerasdf" style="display : none;">
							<div class="signBtnss" id="${sign}" style="position : absolute; width : 50px; height : 50px; border-radius:4px; border : 1px solid black; cursor:pointer;">
								<img src="/files/sign/${sign}" style="width : 100%;">
							</div>
							<div class="stampBtnss" id="${stamp}" style="position : absolute; width : 50px; height : 50px; left : 55px; border-radius:4px; border : 1px solid black; cursor:pointer;">
								<img src="/files/sign/${stamp}" style="width : 100%;">
							</div>
						</div>
					</div>
							...
							<c:forEach var="listp" items="${details.process_line}">
								<tr>
									...
									<td>
										...
										<input type="hidden" name="signiture" value="${stmap.signiture}">
										<input type="hidden" name="lastsigniture">
									<td>
								</tr>
							</c:forEach>
							</tbody>
   ...
   <div class="modal-uploadContract-wrapper">
      <div class="modal-uploadContract-content">
         ...
         <div class="modal-uploadContract-content__footer">
            <a id="download-a" download class="active-yellow btnUploadContract">작성하기</a>
         </div>
